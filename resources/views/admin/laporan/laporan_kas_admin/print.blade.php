...
<body id="container-absolut">
    <div id="app">
        ...
        <table class="table table-bordered" id="tabel-data-kas-admin-print">
            <thead>
                <tr>
                    <th class="text-center">No</th>
                    <th class="text-center">Nama Penyewa</th>
                    <th class="text-center">Jenis</th>
                    <th class="text-center">Kas Admin Masuk</th>
                    <th class="text-center">Kas Admin Keluar</th>
            ...
            <tbody>
                @foreach ($laporan_kas_admin as $key => $lka)
                    <tr>
                        ...
                        <td class="ps-4" id="kas_admin_masuk">{{ $lka->kas_admin_masuk }}</td>
                        <td class="ps-4" id="kas_admin_keluar">{{ $lka->kas_admin_keluar }}</td>
                        <td class="ps-4" id="total_keuangan_admin">{{ $lka->total_kas_admin_semua }}</td>
    ...
    <script>
        ...
        // console.log("LaporanKeuanganAdmin-dari-" + tanggalAwal_format + "-sampai-" + tanggalAkhir_format);

        var workbook = XLSX.utils.book_new();

        // Acquire Data (reference to the HTML table)
        var table_elt = document.getElementById("tabel-data-kas-admin-print");
        ...
        //     Jenis: row.jenis,
        //     Total_Keuangan_Admin: row.total_keuangan_admin,
        //     Tanggal: row.tanggal,
        // }));
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "LaporanKeuanganAdmin");

        /* fix headers */
        XLSX.utils.sheet_add_aoa(worksheet, [
            ["No", "Nama Penyewa", "Jenis", "Keuangan Admin Masuk", "Keuangan Admin Keluar", "Total Keuangan Admin",
                "Tanggal", "Keterangan"
            ]
        ], {
            origin: "A1"
        ...
        worksheet["!cols"] = [{
            ...
            {
                wch: 20
            }, // set column Kas Admin Masuk width 
            {
                wch: 20
            }, // set column Kas Admin Keluar width 
            {
                wch: 20
        ...
        XLSX.writeFile(workbook, "LaporanKeuanganAdmin-dari-" + tanggalAwal_format + "-sampai-" + tanggalAkhir_format +
            ".xlsx", {
                type: "binary",
                cellDates: true,
            });
