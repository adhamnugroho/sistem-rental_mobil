        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <table class="table" id="tabel-data-kas-asuransi">
                    ...
                    <tbody>
                        @foreach ($kas_admin as $key => $ka)
                            <tr>
                                <td class="text-center">{{ $key + 1 }}</td>
                                <td class="ps-4">{{ $ka->penyewa->users->nama_lengkap }}</td>
                                <td class="ps-4">{{ $ka->jenis }}</td>
                                <td class="ps-4">{{ $ka->total_kas_admin_semua }}</td>
                                <td class="text-center">
                                    {{ $ka->tanggal }}
                                </td>
                                <td class="text-center">
                                    <a href="{{ route('keuanganAdminShow', $ka->id_kas_admin) }}"
                                    ...
                                    {{-- @if ($ka->jenis == 'Pengeluaran')
                                        <a href="{{ route('keuanganAdminKurangEdit', $ka->id_kas_admin) }}"
                                    ...
                                    @elseif($ka->jenis == 'Pemasukan')
                                        <a href="{{ route('keuanganAdminTambahEdit', $ka->id_kas_admin) }}"
                                            class="btn btn-primary btn-sm" data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="Tombol Edit Data {{ $judul }}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                    ...
                                    {{-- <button class="btn btn-danger btn-sm" id="button-delete"
                                        onclick="confirmDeleteKeuanganAdmin()" data-bs-toggle="tooltip"
...
@section('script')
    ...
    <script>
        ...
        // $('#button-delete').on('click', confirmDeleteKeuanganAdmin());


        // Konfirmasi Delete Data User
        function confirmDeleteKeuanganAdmin() {

            $('#tabel-data-kas-asuransi tbody').on('click', 'td', function() {

                var id_kas_admin = tabelDataKeuanganAdmin.cell(this, 'id_kas_admin:name', {
                    order: 'original'
                }).data();
                ...
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const hasil_route = route('keuanganAdminDelete', id_kas_admin);
